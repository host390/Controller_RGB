...
void printFromPGM(int charMap, int numberSlaide) {
  ...
  for (uint8_t i = 0; i < printPuncts; i++) { //6 (COLOR_PUNCTS > 6) ? 6 : COLOR_PUNCTS вывод всех пунктов меню // если много пунктов i < 6   - maxParams
    ...
    if (flag_params and modeNum == i + indexDown) { // если 
      for (uint8_t j = 0; j < maxParams; j++) { // выводим каждый параметр
        ...
        if (navPos == 0 and activeNumb == 3 and j == 0 ) { // это чтоб можно было рисовать текст
          oled.print(colorValsHEX[vals[9]]);
        } else if (navPos == 1 and activeNumb + COLOR_PUNCTS == 9 and j == 0) {
          oled.print(colorValsHEX[vals[27]]);
        } else if (navPos == 1 and activeNumb + COLOR_PUNCTS == 9 and j == 1) { 
